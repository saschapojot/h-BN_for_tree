...
class hopping:
    ...
    def __init__(self, to_atom, from_atom, class_id, operation_idx, rotation_matrix, translation_vector, n_vec, is_seed):
        ...
        :param class_id: Integer identifying which equivalence class this hopping belongs to
        ...
        :param n_vec: Array [n₀, n₁, n₂] containing integer coefficients for lattice translation
                  This is the additional lattice shift needed to preserve center atom invariance
        ...
        :param is_seed:  Boolean flag indicating if this is the seed hopping for its equivalence class
                    True for the seed hopping (typically generated by identity operation)
        ...
        self.class_id = class_id  # Equivalence class identifier (immutable integer)
        self.operation_idx = operation_idx  # Which space group operation transforms parent (a reference hopping) to this hopping (immutable)
        ...
        self.n_vec=np.array(n_vec)  # Lattice translation coefficients [n₀, n₁, n₂]
                                      # Ensures the complete transformation: R @ r + t + n₀·a₀ + n₁·a₁ + n₂·a₂
        self.is_seed=is_seed # Boolean: True if this is the seed hopping, False if derived from seed
    ...
    def __repr__(self):
        ...
        :return: str: Compact representation showing: center_type ← neighbor_type,
                 equivalence class ID, operation index, distance, and seed status
        """
...
def search_one_equivalent_atom(target_cart_coord, neighbor_atoms, tolerance=1e-5, verbose=False):
    """
    Search for an atom in the neighbor list whose Cartesian coordinate matches the target.
    ...
    if verbose:
        ...
        print(f"  Searching through {len(neighbor_atoms)} neighbor atoms")
