...
class hopping:
    ...
    def __init__(self, to_atom, from_atom, class_id, operation_idx, rotation_matrix, translation_vector, n_vec, is_seed):
        """
...
def get_next_for_center(center_atom, nghb_atom, space_group_bilbao_cart, operation_idx,
    ...
    1. Check if the center atom is invariant under the space group operation
       (with possible lattice translation). This determines the n_vector.
    2. If invariant, apply the SAME operation (with the SAME n_vector) to the neighbor atom.

    This ensures that the symmetry operation preserves the local environment around
    the center atom, which is crucial for maintaining physical correctness in
    structure analysis.
    ...
    Args:
        ...
        tolerance: Numerical tolerance for invariance check (default: 1e-5)
        verbose: Whether to print debug information (default: False)
    ...
    """
    # Extract the rotation matrix R and translation vector b from the space group operation
    R, b = get_rotation_translation(space_group_bilbao_cart, operation_idx)

    # Get the primitive lattice basis vectors from configuration
    lattice_basis = np.array(parsed_config['lattice_basis'])
    ...
    # STEP 1: Check if center atom is invariant under this operation
    # This determines the lattice translation n_vec = [n0, n1, n2] needed for invariance
    # The full operation is: R @ r + b + n0*a0 + n1*a1 + n2*a2
    ...
    if is_invariant:
        # STEP 2: Apply the SAME symmetry operation to the neighbor atom
        # Crucially, we use the SAME n_vec that made the center atom invariant
        # This preserves the relative geometry of the local environment
        nghb_cart_coord = nghb_atom.cart_coord
        next_cart_coord =bilbao_plus_translation(R,b,lattice_basis,n_vec,nghb_cart_coord)
        ...
        # Center atom is not invariant, so this operation doesn't preserve the local environment
        # Return None to indicate this operation is not valid for this center-neighbor pair
        if verbose:
...
print("center_atom:")
print(center_atom)
bv,nv=check_center_invariant(center_atom,1,space_group_bilbao_cart,np.array(parsed_config['lattice_basis']),1e-5,True)
print(f"bv={bv}, nv={nv}")